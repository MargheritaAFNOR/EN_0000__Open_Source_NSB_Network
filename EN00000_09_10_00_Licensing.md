  ...
  Section:          -- / Licencing
  ----------------- -----------------------------------------------------
  Version:          0.1 -- 260421 mr: Link added
                    0.0 -- 251105 mr: Template added
...
Choosing the right lincence can be assisted by:
...
## Licencing of Pictures

### (Copyright) permissibility of the use of freely available images

The mere fact that an image or graphic is “freely” available on the internet does not mean that its use (e.g. reproduction, distribution) is permitted without the prior consent of the author/rights holder. 

As a general rule, the author must be named as the source. This also applies if the use of the work has been authorised by the author/rights holder. Waiving the requirement to name the author is only possible with the author's express consent. You should therefore also request that the authors of the graphics waive the requirement to name them/cite them as the source. This will allow the images to be used without a link to their direct source.

** The use of protected graphics is not permitted. **



### Neutrality/Compliance

Furthermore, the use of images of products from certain manufacturers could raise questions about the neutrality of the NSBs. Accordingly, it is recommended to refrain from using images with products and brand names.
In principle, the public perception of NSBs as neutral standardisation organisations should not be jeopardised.

Furthermore, it is generally not permitted to link directly to manufacturer websites or online shops. This applies to both direct links and links in plain text. 

Such references could be interpreted as commercial advertising, which would not be readily compatible with the non-profit status of many NSBs. An exception could only apply if it were a source reference within the scope of a permissible quotation.

Finally, care must be taken to ensure that statements and publications on this repository cannot be misunderstood and therefore give the impression of topics that are inadmissible under competition law. 
